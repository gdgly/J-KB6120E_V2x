...
static	BOOL	Print_File_R24_SHI( enum enumSamplerSelect SamplerSelect, uint16_t FileNum, struct uFile_TSP_SHI_R24 const * pFile )
{
	...
	sprintf( sbuffer, "设置采样时间:  %02u:%02u",	pFile->set_time / 60u, pFile->set_time % 60u );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "设置采样流量:  %5.3f L/m",	pFile->set_flow * 0.001f );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "累计采样时间:  %02u:%02u",	pFile->sum_min / 60u, pFile->sum_min % 60u );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "采样体积(标):  %7.1fL",		pFile->vnd );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "平均流量(标):  %6.3f L/m",	(FP32)pFile->vnd / pFile->sum_min );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "采样体积(工):  %7.1fL",		pFile->vd );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "平均流量(工):  %6.3f L/m",	(FP32)pFile->vd / pFile->sum_min );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "平均计前温度:  %6.2f ℃",	pFile->sum_tr / pFile->sum_min );
	PrinterPutString( sbuffer );
	sprintf( sbuffer, "平均计前压力:  %6.2f kPa",	pFile->sum_pr / pFile->sum_min );
...
void	Query_File_TSP( void )
{
	...
	do
	{
		...
		{
			Lputs( 0x0102u, "粉尘文件" );
			ShowI16U( 0x0114u, FileNum, 0x0300u, "[空]" );
		}
		else
		{
			Lputs( 0x0102u, "粉尘文件" );
			ShowI16U( 0x0114u, FileNum, 0x0300u, NULL );
			...
			switch ( option )
			{
				...
				case opt_tsp_1:
					Lputs( 0x011Bu, "[1/2]" );
					Lputs( 0x0602u, "开始时间:" );
					ShowDATE( 0x0610u, File.sample_begin );
					Lputs( 0x0A02u, "采样次数:" );
					ShowI16U( 0x0A16u, File.run_loops,  						0x0200u, NULL );
					Lputs( 0x0E02u, "设置次数:" );
					ShowI16U( 0x0E16u, File.set_loops,   						0x0200u, NULL );
					Lputs( 0x1202u, "温   度:" );
					ShowFP32( 0x1215u, File.sum_tr / File.sum_min, 	0x0402u, "℃" );
					Lputs( 0x1602u, "压   力:" );
					ShowFP32( 0x1617u, File.sum_pr / File.sum_min, 	0x0402u, "kPa" );
					Lputs( 0x1A02u, "大气压力:" );
					ShowFP32( 0x1A15u, File.sum_Ba / File.sum_min,	0x0402u, "kPa" );
					//	Lputs( 0x0800u, "环境温度:" );	ShowFP32( 0x0811u, File.sum_tr / File.sum_min, 0x0602u, "℃" );
					break;
				case opt_tsp_2:
					Lputs( 0x011Bu, "[2/2]" );
					Lputs( 0x0602u, "工   况:" );
					ShowFP32( 0x0617u, File.vd     / File.sum_min, 0x0401u, "L/m" );
					Lputs( 0x0A02u, "标   况:" );
					ShowFP32( 0x0A17u, File.vnd    / File.sum_min, 0x0401u, "L/m" );
					Lputs( 0x0E02u, "设   置:" );
					ShowI16U( 0x0E15u, File.set_flow,              0x0401u, "L/m" );
					Lputs( 0x1202u, "工况体积:" );
					ShowFP32( 0x1213u, File.vd,                    0x0600u, "L" );
					Lputs( 0x1602u, "标况体积:" );
					ShowFP32( 0x1613u, File.vnd,                   0x0600u, "L" );
					Lputs( 0x1A02u, "采样时间:" );
					ShowTIME( 0x1A16u, File.sum_min  );
...
// 					break;
			...
			switch ( option )
			{
				...
				case opt_A_1:
					...
					ShowDATE( 0x0610u, File.sample_begin );
					Lputs( 0x0C02u, "大气压力:" );
					ShowFP32( 0x0C11u, File.sum_Ba / File.sum_min, 0x0602u, "kPa" );
					Lputs( 0x1202u, "采样次数:" );
					ShowI16U( 0x1216u, File.run_loops,   0x0200u, NULL );
					Lputs( 0x1802u, "设置次数:" );
					ShowI16U( 0x1816u, File.set_loops,   0x0200u, NULL );
					break;
				case opt_A_A1:
					Lputs( 0x011Bu, "[2/2]" );
					Lputs( 0x0602u, "温 度:" );
					ShowFP32( 0x0615u, File.sum_tr/ File.sum_min, 	0x0402u, "℃" );
					Lputs( 0x0A02u, "压 力:" );
					ShowFP32( 0x0A15u, File.sum_pr / File.sum_min, 	0x0402u, "kPa" );
					Lputs( 0x0E02u, "标况流量:" );
					ShowFP32( 0x0E15u, File.vnd / File.sum_min, 		0x0403u, "L/m" );
					Lputs( 0x1202u, "标况体积:" );
					ShowFP32( 0x1213u, File.vnd,  									0x0602u, "L " );
					Lputs( 0x1602u, "采样时间:" );
					ShowTIME( 0x1615u, File.sum_min  );
					Lputs( 0x1A02u, "设置时间:" );
					ShowTIME( 0x1A15u, File.set_time );
					break;
			}
		}
...
void	PowerLog_Query( void )
{
	...
	do
	{
		...
		Lputs( 0x1002u, "开机:" );
		ShowDATE( 0x100Cu, Plog.boot );
		Lputs( 0x1802u, "关机:" );

		if ( PLindex )
		{
			ShowDATE( 0x180Cu, Plog.shut );
		}
...
void	menu_SampleQuery( void )	//enum enumSamplerSelect SamplerSelect )
{
	switch ( SamplerSelect )
	{
		case SP_TSP:
			...
			break;
		case SP_R24_A:
			Query_File_SHI_R24(SamplerSelect );
			break;
		case SP_R24_B:
			Query_File_SHI_R24(SamplerSelect );
			break;
