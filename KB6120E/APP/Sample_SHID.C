void  _task_Sample_SHI_D( void )
{
	...
	if ( 0u != start )
	{
		...
		{	//	初始化采样任务
			...
			{
			case enumByDelay:
				start = start + SampleSet[SamplerSelect].delayt;
				break;
			default:
			case enumByAccurate:
				{
					...
					if ( start_hour_minute > SampleSet[SamplerSelect].delayt )
					{
						start += 1440u;	//	推迟到明天的这个时间采样
					}
					start = start - start_hour_minute + SampleSet[SamplerSelect].delayt;
				}
				break;
			}
			iloop = 1u;
		...
		while (( ! SampleSwitch[SamplerSelect].Clean ) && ( iloop <= SampleSet[SamplerSelect].set_loops ))
		{
			...
			while (( ! SampleSwitch[SamplerSelect].Clean ) && ( now_minute < start ))
			{
				...
				if ( PumpState != Q_Sampler[SamplerSelect].xp_state )
				{
					Pump_OutCmd( SamplerSelect, PumpState );
					Q_Sampler[SamplerSelect].xp_state = PumpState;
				}
